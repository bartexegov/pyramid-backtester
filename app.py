        ...
        if True:
            ...
            with chart_tab3:
                ...
                <div style='background:#0c2541;border:1px solid #0369a1;border-radius:8px;padding:14px 18px;margin-top:8px;font-size:0.82rem;color:#93c5fd;line-height:1.7'>
                    <b style='color:#38bdf8'>Quick start (IBKR):</b><br>
                ...
                # ── NinjaScript section ────────────────────────
                st.markdown("<div style='height:1px;background:#1e293b;margin:24px 0 16px 0'></div>", unsafe_allow_html=True)
                st.markdown("<div style='font-size:0.7rem;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:12px'>NinjaTrader 8 — NinjaScript Strategy v2</div>", unsafe_allow_html=True)
                st.markdown("""
                <div style='background:#1e293b;border:1px solid #334155;border-radius:10px;padding:14px 16px;margin-bottom:12px;font-size:0.82rem;color:#94a3b8;line-height:1.7'>
                    <b style='color:#34d399'>v2 — fixed version:</b> Uses <b>OnEachTick</b> (reacts immediately),
                    <b>ticks instead of price</b> for TP (more reliable in NT8),
                    simplified state management, proper TP tracking via <b>Position.AveragePrice</b>.<br>
                    <b style='color:#fbbf24'>Always test on Simulated account first!</b>
                </div>
                """, unsafe_allow_html=True)

                # Calculate ticks from dollar values
                # For ZC: tick = 0.25 cents, so 5 cents = 20 ticks
                # We store tick size per instrument for the conversion hint
                _tick_info = {
                    "Corn (ZC)":          (0.25, "cents"),
                    "Wheat (ZW)":         (0.25, "cents"),
                    "Soybeans (ZS)":      (0.25, "cents"),
                    "Oats (ZO)":          (0.25, "cents"),
                    "Rough Rice (ZR)":    (0.005,"cents/cwt"),
                    "Sugar #11 (SB)":     (0.01, "cents/lb"),
                    "Cotton (CT)":        (0.01, "cents/lb"),
                    "Coffee (KC)":        (0.05, "cents/lb"),
                    "Cocoa (CC)":         (1.0,  "USD/ton"),
                    "Orange Juice (OJ)":  (0.05, "cents/lb"),
                    "Live Cattle (LE)":   (0.025,"cents/lb"),
                    "Lean Hogs (HE)":     (0.025,"cents/lb"),
                    "Feeder Cattle (FC)": (0.025,"cents/lb"),
                    "Crude Oil (CL)":     (0.01, "USD/bbl"),
                    "Natural Gas (NG)":   (0.001,"USD/MMBtu"),
                    "Gold (GC)":          (0.10, "USD/oz"),
                    "Silver (SI)":        (0.005,"cents/oz"),
                    "Palladium (PA)":     (0.05, "USD/oz"),
                    "Platinum (PL)":      (0.10, "USD/oz"),
                    "Copper (HG)":        (0.0005,"cents/lb"),
                    "S&P 500 (ES)":       (0.25, "pts"),
                    "Nasdaq (NQ)":        (0.25, "pts"),
                }
                tick_sz, tick_unit = _tick_info.get(_sym, (0.25, "units"))
                try:
                    step_ticks = max(1, int(round(float(_step) / tick_sz)))
                    tp_ticks   = max(1, int(round(float(_tp)   / tick_sz)))
                except Exception:
                    step_ticks = 20
                    tp_ticks   = 20

                # Hint to user
                st.info(f"**{_sym}**: tick size = {tick_sz} {tick_unit}  →  Step {_step} = **{step_ticks} ticks** | TP {_tp} = **{tp_ticks} ticks**")
                ...
                ninja_v2 = (
                    ...
                    '#endregion\n'
                    '\n'
                    '// PyramidStrategy v2 — Auto-generated by Pyramid Backtester\n'
                    '// Instrument : __SYM__   Direction: __DIR__\n'
                    '// Threshold  : __THR__   Step: __STEP_TICKS__ ticks   TP: __TP_TICKS__ ticks\n'
                    '// OnEachTick + CalculationMode.Ticks + Position.AveragePrice\n'
                    '\n'
                    'namespace NinjaTrader.NinjaScript.Strategies\n'
                    '{\n'
                    '    public class PyramidStrategy : Strategy\n'
                    '    {\n'
                    '        private double lastEntryPrice  = double.NaN;\n'
                    '        private int    openLevels      = 0;\n'
                    '        private bool   strategyActive  = false;\n'
                    '\n'
                    '        [NinjaScriptProperty] public string Direction        { get; set; }\n'
                    '        [NinjaScriptProperty] public double EntryThreshold   { get; set; }\n'
                    '        [NinjaScriptProperty] public int    PyramidStepTicks { get; set; }\n'
                    '        [NinjaScriptProperty] public int    TakeProfitTicks  { get; set; }\n'
                    '        [NinjaScriptProperty] public int    QtyPerEntry      { get; set; }\n'
                    '        [NinjaScriptProperty] public int    MaxLevels        { get; set; }\n'
                    '\n'
                    '        protected override void OnStateChange()\n'
                    '        {\n'
                    '            if (State == State.SetDefaults)\n'
                    '            {\n'
                    '                Name                         = "PyramidStrategy";\n'
                    ...
                    '                TraceOrders                  = true;\n'
                    '                Direction        = "__DIR__";\n'
                    '                EntryThreshold   = __THR__;\n'
                    '                PyramidStepTicks = __STEP_TICKS__;\n'
                    '                TakeProfitTicks  = __TP_TICKS__;\n'
                    '                QtyPerEntry      = 1;\n'
                    '                MaxLevels        = 20;\n'
                    '            }\n'
                    '            else if (State == State.Configure || State == State.Transition)\n'
                    '            { lastEntryPrice = double.NaN; openLevels = 0; strategyActive = false; }\n'
                    '        }\n'
                    '\n'
                    ...
                    '            bool isLong = Direction.Trim().ToLower() == "long";\n'
                    '            double price = Close[0];\n'
                    '            if (Math.Abs(Position.Quantity) == 0 && strategyActive)\n'
                    '            { lastEntryPrice = double.NaN; openLevels = 0; strategyActive = false;\n'
                    '              Print(Time[0] + " [RESET] Flat. Waiting for next signal."); }\n'
                    '            if (isLong)\n'
                    '            {\n'
                    '                if (openLevels == 0 && price < EntryThreshold)\n'
                    '                    OpenLevel(isLong, price);\n'
                    '                else if (openLevels > 0 && openLevels < MaxLevels && !double.IsNaN(lastEntryPrice)\n'
                    '                         && price <= lastEntryPrice - PyramidStepTicks * TickSize)\n'
                    '                    OpenLevel(isLong, price);\n'
                    '            }\n'
                    '            else\n'
                    '            {\n'
                    ...
                    '        }\n'
                    '\n'
                    '        private void OpenLevel(bool isLong, double price)\n'
                    '        {\n'
                    '            openLevels++; strategyActive = true;\n'
                    '            string id = "L" + openLevels.ToString();\n'
                    '            Print(string.Format("{0} [ENTRY L{1}] {2} @ {3:F4}",\n'
                    '                Time[0], openLevels, isLong ? "BUY" : "SELL SHORT", price));\n'
                    '            if (isLong) { EnterLong(QtyPerEntry, id);  SetProfitTarget(id, CalculationMode.Ticks, TakeProfitTicks); }\n'
                    '            else        { EnterShort(QtyPerEntry, id); SetProfitTarget(id, CalculationMode.Ticks, TakeProfitTicks); }\n'
                    '            lastEntryPrice = price;\n'
                    '        }\n'
                    '\n'
                    '        protected override void OnExecutionUpdate(Execution e, string eid, double price, int qty, MarketPosition mp, string oid, DateTime t)\n'
                    '        {\n'
                    '            if (e.Order == null) return;\n'
                    '            bool tpFill = (e.Order.OrderAction == OrderAction.Sell && Direction.Trim().ToLower() == "long") ||\n'
                    '                          (e.Order.OrderAction == OrderAction.BuyToCover && Direction.Trim().ToLower() != "long");\n'
                    '            if (tpFill)\n'
                    '            {\n'
                    '                openLevels = Math.Max(0, openLevels - 1);\n'
                    '                lastEntryPrice = openLevels == 0 ? double.NaN : Position.AveragePrice;\n'
                    '                Print(string.Format("{0} [TP HIT] Remaining: {1}  AvgEntry: {2:F4}", t, openLevels, Position.AveragePrice));\n'
                    '            }\n'
                    '        }\n'
                    '    }\n'
                    '}\n'
                    ...
                    '// ZC Corn:    0.25 cents/tick  ->  5 cents = 20 ticks\n'
                    '// GC Gold:    0.10 USD/tick    ->  5 USD   = 50 ticks\n'
                    '// CL Crude:   0.01 USD/tick    ->  5 USD   = 500 ticks\n'
                    '// ES S&P500:  0.25 pts/tick    ->  5 pts   = 20 ticks\n'
                    '// Formula: ticks = dollar_amount / tick_size\n'
                )
                ...
                ninja_filled = (
                    ...
                    .replace("__SYM__",        str(_sym))
                    .replace("__THR__",        str(_thr))
                    .replace("__STEP_TICKS__", str(step_ticks))
                    .replace("__TP_TICKS__",   str(tp_ticks))
                )

                st.code(ninja_filled, language="csharp")
                st.download_button(
                    label="⬇️ Download PyramidStrategy.cs",
                    data=ninja_filled,
                    file_name="PyramidStrategy.cs",
                    mime="text/plain",
                    use_container_width=True,
                )
                st.markdown("""
                <div style='background:#0c2541;border:1px solid #0369a1;border-radius:8px;padding:14px 18px;margin-top:8px;font-size:0.82rem;color:#93c5fd;line-height:1.7'>
                    <b style='color:#38bdf8'>Quick start (NinjaTrader 8):</b><br>
                    1. Tools → NinjaScript Editor → right-click Strategies → New → name: PyramidStrategy<br>
                    2. Delete all default code, paste this file, press F5 (Compile)<br>
                    3. Open chart → right-click → Strategies → Add Strategy → PyramidStrategy<br>
                    4. Parameters are pre-filled — click Enable<br>
                    5. Check Output Window (Ctrl+Alt+O) — you should see [ENTRY L1] messages
                </div>
                """, unsafe_allow_html=True)
