    ...
    if "bt_result" not in st.session_state:
        ...
        if True:
            ...
            with chart_tab3:
                ...
                st.markdown("<div style='font-size:0.7rem;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:12px'>NinjaTrader 8 — NinjaScript Strategy</div>", unsafe_allow_html=True)
                st.markdown("""
                <div style='background:#1e293b;border:1px solid #334155;border-radius:10px;padding:14px 16px;margin-bottom:12px;font-size:0.82rem;color:#94a3b8;line-height:1.7'>
                    Same strategy logic as the IBKR script above — runs natively inside NinjaTrader 8.
                    Each pyramid entry gets its own <b style='color:#38bdf8'>SetProfitTarget()</b> order.
                    Works on any instrument available in NT8 (Futures, Stocks, Forex).
                    <b style='color:#fbbf24'>Always test on Simulated account first!</b>
                </div>
                """, unsafe_allow_html=True)

                # Generate NinjaScript with current backtest parameters
                ninja_script = (
                    ...
                    '#endregion\n'
                    '\n'
                    '// Pyramid Strategy — Auto-generated by Pyramid Backtester\n'
                    '// Instrument : __SYM__\n'
                    '// Direction  : __DIR__\n'
                    '// Threshold  : __THR__  Step: __STEP__  TP: __TP__\n'
                    '// HOW TO USE:\n'
                    '//   1. Tools -> NinjaScript Editor -> right-click Strategies -> New\n'
                    '//   2. Name it "PyramidStrategy", paste this code, press F5 to compile\n'
                    '//   3. Right-click chart -> Strategies -> Add Strategy -> PyramidStrategy\n'
                    '//   4. Set parameters and Enable\n'
                    '\n'
                    'namespace NinjaTrader.NinjaScript.Strategies\n'
                    '{\n'
                    '    public class PyramidStrategy : Strategy\n'
                    '    {\n'
                    '        private System.Collections.Generic.List<double> entryPrices = new System.Collections.Generic.List<double>();\n'
                    ...
                    '                // ── Parameters pre-filled from backtester ──\n'
                    '                Direction      = "__DIR__";\n'
                    '                EntryThreshold = __THR__;\n'
                    '                PyramidStep    = __STEP__;\n'
                    '                TakeProfit     = __TP__;\n'
                    '                QtyPerEntry    = 1;\n'
                    '                MaxLevels      = 20;\n'
                    '            }\n'
                    ...
                    '            {\n'
                    '                entryPrices    = new System.Collections.Generic.List<double>();\n'
                    '                lastEntryPrice = double.NaN;\n'
                    '                entryCount     = 0;\n'
                    '            }\n'
                    '        }\n'
                    '\n'
                    ...
                    '        [NinjaScriptProperty] public double PyramidStep    { get; set; }\n'
                    '        [NinjaScriptProperty] public double TakeProfit     { get; set; }\n'
                    ...
                    '            if (Math.Abs(Position.Quantity) == 0 && entryCount > 0) ResetState();\n'
                    '            double price = Close[0];\n'
                    '            if (isLong)\n'
                    '            {\n'
                    '                if (entryCount == 0 && price < EntryThreshold)\n'
                    '                    PlaceEntry(true, price);\n'
                    '                else if (entryCount > 0 && entryCount < MaxLevels && !double.IsNaN(lastEntryPrice))\n'
                    '                    if (price <= lastEntryPrice - PyramidStep)\n'
                    '                        PlaceEntry(true, price);\n'
                    '            }\n'
                    '            else\n'
                    '            {\n'
                    '                if (entryCount == 0 && price > EntryThreshold)\n'
                    '                    PlaceEntry(false, price);\n'
                    '                else if (entryCount > 0 && entryCount < MaxLevels && !double.IsNaN(lastEntryPrice))\n'
                    '                    if (price >= lastEntryPrice + PyramidStep)\n'
                    '                        PlaceEntry(false, price);\n'
                    '            }\n'
                    '        }\n'
                    '\n'
                    '        private void PlaceEntry(bool isLong, double price)\n'
                    '        {\n'
                    '            entryCount++;\n'
                    '            string id = "L" + entryCount.ToString();\n'
                    '            double tp = isLong ? price + TakeProfit : price - TakeProfit;\n'
                    '            Print(string.Format("[ENTRY #{0}] {1} @ {2:F4}  TP={3:F4}", entryCount, isLong?"BUY":"SELL", price, tp));\n'
                    '            if (isLong) { EnterLong(QtyPerEntry, id); SetProfitTarget(id, CalculationMode.Price, tp); }\n'
                    '            else        { EnterShort(QtyPerEntry, id); SetProfitTarget(id, CalculationMode.Price, tp); }\n'
                    '            entryPrices.Add(price);\n'
                    '            lastEntryPrice = price;\n'
                    '        }\n'
                    '\n'
                    '        protected override void OnExecutionUpdate(Execution e, string executionId, double price, int qty, MarketPosition mp, string orderId, DateTime time)\n'
                    '        {\n'
                    '            if (e.Order == null) return;\n'
                    '            if (e.Order.Name.StartsWith("Profit target"))\n'
                    '            {\n'
                    '                for (int i = entryPrices.Count - 1; i >= 0; i--)\n'
                    '                {\n'
                    '                    if (e.Order.Name.Contains("L" + (i+1).ToString()))\n'
                    '                    {\n'
                    '                        Print(string.Format("[TP HIT] L{0} @ {1:F4}", i+1, price));\n'
                    '                        entryPrices.RemoveAt(i);\n'
                    '                        entryCount = Math.Max(0, entryCount - 1);\n'
                    '                        if (entryPrices.Count == 0) ResetState();\n'
                    '                        else lastEntryPrice = Direction.Trim().ToLower()=="long" ? Min(entryPrices) : Max(entryPrices);\n'
                    '                        break;\n'
                    '                    }\n'
                    '                }\n'
                    '            }\n'
                    '        }\n'
                    '\n'
                    '        private void ResetState() { entryPrices.Clear(); lastEntryPrice = double.NaN; entryCount = 0; Print("[RESET] Waiting for next signal."); }\n'
                    '        private double Min(System.Collections.Generic.List<double> l) { double m=l[0]; foreach(double v in l) if(v<m)m=v; return m; }\n'
                    '        private double Max(System.Collections.Generic.List<double> l) { double m=l[0]; foreach(double v in l) if(v>m)m=v; return m; }\n'
                    '    }\n'
                    '}\n'
                )
                ...
                <div style='background:#0c2541;border:1px solid #0369a1;border-radius:8px;padding:14px 18px;margin-top:8px;font-size:0.82rem;color:#93c5fd;line-height:1.7'>
                    ...
                    2. Select all default code and paste this file<br>
                    3. Press F5 (Compile) — check Output window for errors<br>
                    4. Open chart → right-click → Strategies → Add Strategy → PyramidStrategy<br>
                    5. Parameters are pre-filled — click Enable to start
                </div>
                """, unsafe_allow_html=True)
